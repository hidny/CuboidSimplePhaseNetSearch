...
public class RegionSplitLogicSimple4 {

	private CoordWithRotationAndIndex[][] neighbours;

	public RegionSplitLogicSimple4(CoordWithRotationAndIndex[][] neighbours) {
	...
	public void initNumCellsAbovePerLayer() {
		...
		for(int i=0; i<numCellsAbovePerLayerStateMid.length; i++) {
			...
			for(int j=0; j<CuboidToFoldOnExtendedSimplePhase5.CELLS_TO_ADD_BY_STATE_GOING_UP_MIDDLE[i].length; j++) {
				
				if(! foundFirst && CuboidToFoldOnExtendedSimplePhase5.CELLS_TO_ADD_BY_STATE_GOING_UP_MIDDLE[i][j] == 1) {
					firstIndex = j;
					foundFirst = true;

				} else if(foundFirst && CuboidToFoldOnExtendedSimplePhase5.CELLS_TO_ADD_BY_STATE_GOING_UP_MIDDLE[i][j] == 0) {
					lastIndex = j;
					break;
				}
			}
			if(lastIndex == -1) {
				lastIndex = CuboidToFoldOnExtendedSimplePhase5.CELLS_TO_ADD_BY_STATE_GOING_UP_MIDDLE[i].length;
			}
		...
		for(int i=0; i<numCellsAbovePerLayerStateSide.length; i++) {
			...
			for(int j=0; j<CuboidToFoldOnExtendedSimplePhase5.CELLS_TO_ADD_BY_STATE_GOING_UP_ON_SIDE[i].length; j++) {
				
				if(! foundFirst && CuboidToFoldOnExtendedSimplePhase5.CELLS_TO_ADD_BY_STATE_GOING_UP_ON_SIDE[i][j] == 1) {
					firstIndex = j;
					foundFirst = true;

				} else if(foundFirst && CuboidToFoldOnExtendedSimplePhase5.CELLS_TO_ADD_BY_STATE_GOING_UP_ON_SIDE[i][j] == 0) {
					lastIndex = j;
					break;
				}
