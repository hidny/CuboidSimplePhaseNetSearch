...
12 x 3 x 1: 102
	...
	public static void reallySimpleSearch(int a, int b, int c) {
		...
		CuboidToFoldOnExtendedSimplePhase5 cuboidToBuild = new CuboidToFoldOnExtendedSimplePhase5(a, b, c);
		...
		for(int i=0; i<startingPointsAndRotationsToCheck.size(); i++) {
			...
			cuboidToBuild = new CuboidToFoldOnExtendedSimplePhase5(a, b, c);
	...
	public static int getNumLayers(CuboidToFoldOnExtendedSimplePhase5 cuboidToBuild) {
		
		return (cuboidToBuild.getNumCellsToFill() - 2) / 4;
	}
	
	public static long findReallySimpleSolutionsRecursion(CuboidToFoldOnExtendedSimplePhase5 cuboidToBuild) {
	...
	public static long findReallySimpleSolutionsRecursionFirstLayer(CuboidToFoldOnExtendedSimplePhase5 cuboidToBuild, int numLayers) {
		long ret = 0;
		...
		for(int sideBump=FIRST_LEGAL_SIDE_BUMP_Nx1x1; sideBump <FIRST_LEGAL_SIDE_BUMP_Nx1x1 + WIDTH_Nx1x1; sideBump++) {
			...
			if(CuboidToFoldOnExtendedSimplePhase5.LEVEL_OPTIONS[0][0 - (sideBump - 6)] != 1) {
				...
				if( curLayerIndex > 12 || ! cuboidToBuild.untouchableRegionCreatedAfterLayerAdded() ) {
					...
					if(curLayerIndex > 12 || ! cuboidToBuild.twoRegionsCreatedAfterLayerAdded()) {
						...
						if(nextLayerState == 0) {
							...
							//TODO: rename function to be more intuitive (left bigger so go right is tricky)
							if(cuboidToBuild.leftRegionOfTwoRegionsCreatedAfterLayerAddedBigger()) {
								ret += findReallySimpleSolutionsRecursionForcedSideRight(cuboidToBuild, curLayerIndex + 1, numLayers, nextLayerState, debugNope);
							} else {
								ret += findReallySimpleSolutionsRecursionForcedSideLeft(cuboidToBuild, curLayerIndex + 1, numLayers, nextLayerState, debugNope);
							}

						} else if(nextLayerState > 0 && nextLayerState <=3) {
							ret += findReallySimpleSolutionsRecursionForcedSideLeft(cuboidToBuild, curLayerIndex + 1, numLayers, nextLayerState, debugNope);
		
						} else if(nextLayerState > 3 && nextLayerState <=6) {
							ret += findReallySimpleSolutionsRecursionForcedSideRight(cuboidToBuild, curLayerIndex + 1, numLayers, nextLayerState, debugNope);
						}
					}
	...
	public static long findReallySimpleSolutionsRecursionForcedSideLeft(CuboidToFoldOnExtendedSimplePhase5 cuboidToBuild, int curLayerIndex, int numLayers, int prevLayerStateIndex,
			boolean debugNope) {
		...
		//Go faster by iterating through the possible moves (i.e. (nextLayerState, sideBump) tuples)
		for(int i=0; i<cuboidToBuild.getNextLayerPossibilitiesForcedLeft()[prevLayerStateIndex].length; i++) {

			int nextLayerState = cuboidToBuild.getNextLayerPossibilitiesForcedLeft()[prevLayerStateIndex][i].i;
			int sideBump = cuboidToBuild.getNextLayerPossibilitiesForcedLeft()[prevLayerStateIndex][i].j;
			...
			if(cuboidToBuild.isNewLayerValidSimpleFast(nextLayerState, sideBump)) {
				...
				if( ! cuboidToBuild.untouchableRegionCreatedAfterLayerAdded() ) {
					...
					if(cuboidToBuild.isolatedSideRegionMaybeFilledUp()) {
						ret += findReallySimpleSolutionsRecursion(cuboidToBuild, curLayerIndex + 1, numLayers, nextLayerState, debugNope);
					} else {
						ret += findReallySimpleSolutionsRecursionForcedSideLeft(cuboidToBuild, curLayerIndex + 1, numLayers, nextLayerState, debugNope);
					}
	...
	public static long findReallySimpleSolutionsRecursionForcedSideRight(CuboidToFoldOnExtendedSimplePhase5 cuboidToBuild, int curLayerIndex, int numLayers, int prevLayerStateIndex,
			boolean debugNope) {
		...
		//Go faster by iterating through the possible moves (i.e. (nextLayerState, sideBump) tuples)
		for(int i=0; i<cuboidToBuild.getNextLayerPossibilitiesForcedRight()[prevLayerStateIndex].length; i++) {

			int nextLayerState = cuboidToBuild.getNextLayerPossibilitiesForcedRight()[prevLayerStateIndex][i].i;
			int sideBump = cuboidToBuild.getNextLayerPossibilitiesForcedRight()[prevLayerStateIndex][i].j;
