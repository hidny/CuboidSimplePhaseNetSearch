...
public class MatrixCreator6 {
	...
	public static void main(String args[]) {
		...
		ArrayList <LayerState6> validLayerStates = getValidLayerStates();
	...
	public static ArrayList <LayerState6> getValidLayerStates() {
		
		//Hashtable <String, LayerState> validLayerStates = new Hashtable<String, LayerState>();

		ArrayList<LayerState6> validLayerStates = new ArrayList<LayerState6>();
		HashSet<LayerState6> validLayerStatesHash = new HashSet<LayerState6>();
		
		LinkedList<LayerState6> layerStateQueue = new LinkedList<LayerState6>();
		
		//Start with the fully connected layer:
		LayerState6 currentBottomLayer = new LayerState6(PERIMETER, 0);
		...
		while(layerStateQueue.isEmpty() == false) {
			...
			for(int i=0; i<numLayers; i++) {
				...
				for(int sideBump=LEFT_EXTREME; currentTopLayerWidth + sideBump < currentBottomLayerWidth + PERIMETER; sideBump++) {
					...
					LayerState6 layerAbove = LayerState6.addLayerStateOnTopOfLayerState(currentBottomLayer, stateWithoutConnections, sideBump);
					...
					if(layerAbove != null) {
						...
						//TODO (July 5th: move this elsewhere, so my brain doesn't hurt:
						if( numNonRedundantConnectionsSoFar == halfNonRedundantConnectionsNeededNow
								&& ! couldTouchTopRef.containsKey(layerAbove.toString())
								){
							debugNumCouldReachLayer0_calls++;
							
							
							if(curLayerStateCouldReachLayer0(layerAbove, validLayerStatesHash)) {
								
								//System.out.println("Could reach layer 0: (Number of states that reach layer 0: " + (validLayerStates.size() + 1) + ")");
	...
	public static int debugNumCouldReachLayer0_calls = 0;
	...
	public static boolean curLayerStateCouldReachLayer0(LayerState6 cur, HashSet<LayerState6> validLayerStatesHash) {
		...
		LayerState6 goal = new LayerState6(PERIMETER, 0);
		
		LinkedList<LayerState6> layerStateQueue = new LinkedList<LayerState6>();
		layerStateQueue.add(cur);
		
		Hashtable <String, LayerState6> layerStatesFoundInReachLayer0Search = new Hashtable<String, LayerState6>();
		layerStatesFoundInReachLayer0Search.put(cur.toString(), cur);
		...
		while( ! layerStateQueue.isEmpty()) {
			...
			LayerState6 bottomLayer = layerStateQueue.poll();
			...
			for(int i=0; i<numLayers; i++) {
				...
				foundSomething = searchForLayer(
						...
						topToBottomRecords,
						layerStatesFoundInReachLayer0Search,
						layerStateQueue,
		...
		Enumeration<String> badStates = layerStatesFoundInReachLayer0Search.keys();
	...
	public static boolean searchForLayer(
			LayerState6 bottomLayer,
			HashSet<LayerState6> validLayerStatesHash,
			Hashtable <String, String> topToBottomRecords,
			Hashtable <String, LayerState6> layerStatesFoundInReachLayer0Search,
			LinkedList<LayerState6> layerStateQueue,
			LayerState6 stateWithoutConnections
		) {
		...
		for(int sideBump=LEFT_EXTREME; currentTopLayerWidth + sideBump < currentBottomLayerWidth + PERIMETER; sideBump++) {
			...
			LayerState6 layerAbove = LayerState6.addLayerStateOnTopOfLayerState(bottomLayer, stateWithoutConnections, sideBump);
				...
				if(numNonRedundantConnectionsSoFar == halfNonRedundantConnectionsNeededNow) {
					...
					if(couldTouchTopRef.containsKey(layerAbove.toString())) {
						...
						if(couldTouchTopRef.get(layerAbove.toString()) == true) {
							...
							LayerState6 curMemoize = bottomLayer;
							...
							while(topToBottomRecords.containsKey(curMemoize.toString())) {
								curMemoize = layerStatesFoundInReachLayer0Search.get(topToBottomRecords.get(curMemoize.toString()));
								if(! couldTouchTopRef.contains(curMemoize.toString())) {
									couldTouchTopRef.put(curMemoize.toString(), true);
									//System.out.println("Adding: " + curMemoize.toString());
								}
							...
							//pass because layerAbove doesn't reach state 0
						}
					
					} else if( ! layerStatesFoundInReachLayer0Search.containsKey(layerAbove.toString())) {
						
						layerStatesFoundInReachLayer0Search.put(layerAbove.toString(), layerAbove);
						layerStateQueue.add(layerAbove);
	...
	public static int[][] createMatrix(ArrayList <LayerState6> validLayerStates) {
		
		int ret[][] = new int[validLayerStates.size()][validLayerStates.size()];
		
		
		LayerState6 states[] = new LayerState6[validLayerStates.size()];
