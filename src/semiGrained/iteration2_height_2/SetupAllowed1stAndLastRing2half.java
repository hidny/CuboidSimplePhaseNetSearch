...
public class SetupAllowed1stAndLastRing2half {
	...
	public SetupAllowed1stAndLastRing2half(
		...
		for(int index=0; index<Utils.getTotalArea(this.dimensions); index++) {
			for(int rotation=0; rotation<4; rotation++) {
				...
				if(indexType == CuboidToFoldOnSemiGrained2half.LEFT_TOP_LOCATION) {
					topLeftMostShiftIndex[curTopLefti] = index;
					curTopLefti++;
					
				} else if(indexType == CuboidToFoldOnSemiGrained2half.LEFT_BOTTOM_LOCATION) {
					bottomLeftMostShiftIndex[SIZE - 1 - curBottomLeftj] = index;
					curBottomLeftj++;
					
				} else if(indexType == CuboidToFoldOnSemiGrained2half.RIGHT_TOP_LOCATION) {
					topRightMostShiftIndex[SIZE - 1 - curTopRighti] = index;
					curTopRighti++;
					
				} else if(indexType == CuboidToFoldOnSemiGrained2half.RIGHT_BOTTOM_LOCATION) {
	...
	public boolean areTopShiftIndexesAllSet(CuboidToFoldOnSemiGrained2half sg) {
		for(int i=0; i<topLeftMostShiftIndex.length; i++) {
	...
	public boolean areBottomShiftIndexesAllSet(CuboidToFoldOnSemiGrained2half sg) {
		for(int i=0; i<topLeftMostShiftIndex.length; i++) {
